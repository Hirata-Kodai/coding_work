...
const CLIPS := {
	...
	"high": {"file": DIR + "Lead Jab.fbx", "start": 0.2, "end": 0.9, "loop": false},
	"low": {"file": DIR + "Leg Sweep.fbx", "start": 0.2, "end": 1.3, "loop": false},
	"throw": {"file": DIR + "Goalie Throw.fbx", "start": 0.8, "end": 2.6, "loop": false},
...
static func select(core: FighterCore) -> Dictionary:
	match core.state:
		...
		FighterCore.State.ATTACK:
			return _stretch(MOVE_CLIP[core.move], core.frame, Moves.total_frames(core.move))
